...
### 0.1 已验证事实 (Fact)
1. **源码与运行时依赖**：当前项目后端基于 Spring Boot 3.4.4，编译与运行必须依赖 JDK 21（[pom.xml](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/pom.xml%23L63)）；前端为 Vue CLI 4 项目，构建依赖 Node.js 且需放开 OpenSSL 3.0 兼容参数（[doc/_content/introduction/compile.md](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/doc/_content/introduction/compile.md%23L309)）。
2. **数据源支持现状**：后端已集成 `mysql-connector-j:8.2.0` 驱动（[pom.xml](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/pom.xml%23L168-L172)），原生完全兼容 MySQL / MariaDB 协议；未集成 SQLite JDBC 驱动。
3. **现有 Dockerfile 分布**：目前仓库在 `docker/` 下为分立构建模式（wvp、media、mysql、redis、nginx 分别打包），尚未提供全组件一体化单镜像（[docker/docker-compose.yml](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/docker/docker-compose.yml)）。
...
1. **一体化单镜像数据库引擎选型**：
   ...
   - Alpine `mariadb` 与 `mariadb-client` 安装包仅增加 ~18MB 压缩体积，完全兼容现有 MySQL 8.0 语法与全套建表脚本（[数据库/2.7.4/初始化-mysql-2.7.4.sql](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/%E6%95%B0%E6%8D%AE%E5%BA%93/2.7.4/%E5%88%9D%E5%A7%8B%E5%8C%96-mysql-2.7.4.sql)），工程师无需改变任何 SQL 操作习惯或维护两套方言脚本。
2. **进程管理与信号级联守护**：
...
| **ONVIF 发现与信令** | ONVIF 摄像头 $\leftrightarrow$ WVP | OASIS WS-Discovery<br>ONVIF Profile S/T (SOAP) | 探测端口 `3702/udp`（组播 `239.255.255.250`）。Bridge 网桥会丢弃组播回包，**自动搜寻必须依赖 `--net=host`**；Bridge 模式下支持单播 IP:Port 直连。信令走标准 HTTP SOAP（80/8080/8899）。 |
...
+---------------------------------------------------------------------------------------------------+
       ^                          ^                             ^                          ^                          ^
       | 18080 (Web UI & REST)    | 8116 / 5060 (SIP 信令)       | 3702/udp (ONVIF 搜寻)    | 8000/udp (WebRTC 对讲)    | 30000-30050
       |                          |                             |                          |                          | (RTP 收流)
+-----------------------------------------------------------------------------------------------------------------------------------+
...
- **Task 3.1: 工程师本地一键编排启动 (`docker-compose.yml`)**
  ...
  services:
    wvp-aio:
      ...
      # 注：ONVIF WS-Discovery 局域网组播自动探测必须在此模式下生效；Bridge 模式下仅支持单播指定 IP:Port 添加
      # network_mode: host
      # -------------------------------------------------------------
      ...
      ports:
        ...
        # [核心 3] ONVIF WS-Discovery 局域网设备自动发现 (UDP)
        - "3702:3702/udp"

        # [核心 4] ZLMediaKit HTTP 流媒体点播 (HTTP-FLV / HLS) 与 REST API
        - "9092:9092/tcp"

        # [核心 5] WebRTC 媒体协商与语音对讲核心端口 (必须暴露 UDP)
        - "8000:8000/udp"
        - "8000:8000/tcp"
        ...
        # [核心 7] GB28181 RTP 国标媒体收流端口段（按需调整范围，建议与 zlm.ini 保持一致）
...
- **Task 3.2: 全栈对外端口映射矩阵与防火墙放行规范 (Port Mapping Matrix)**
  ...
  | 3 | **`3702`** | UDP | WVP-PRO (ONVIF) | WS-Discovery 局域网设备自动发现探测与回包接收 | **搜寻必选** (UDP) |
  | 4 | **`9092`** | TCP | ZLMediaKit | 流媒体 HTTP REST API、HTTP-FLV / HLS / WebSocket-FLV 实时播放源 | **必选放行** |
  | 5 | **`8000`** | UDP & TCP | ZLMediaKit | WebRTC 媒体流收发、**双向语音对讲**、浏览器端超低延迟点播（毫秒级） | **对讲必选** (UDP 为核心) |
  | 6 | **`1935`** | TCP & UDP | ZLMediaKit | RTMP 实时视频流发布与拉取，兼容 OBS / FFmpeg 常见推流工具 | 可选放行 |
  | 7 | **`554`** | TCP & UDP | ZLMediaKit | RTSP 实时流分发与拉流代理（支持 VLC / 监控大屏拉流） | 可选放行 |
  | 8 | **`30000~30050`** | UDP & TCP | ZLMediaKit (RTP) | GB28181 国标推流端口池，用于接收摄像头 PS-RTP 音视频数据流（多端口并发） | **推流必选** (范围可伸缩) |
  | 9 | **`3306`** | TCP | MariaDB | 数据库管理端口，支持 Navicat / DBeaver 本地直连调试 | **仅内网/调试暴露** |
  | 10 | **`6379`** | TCP | Redis | 缓存管理端口，支持 RedisInsight 查看 Session / 心跳键值 | **仅内网/调试暴露** |

  > [!TIP]
  > **Bridge 模式 vs Host 模式深度抉择：**  
  > - **macOS / Windows 配合机**：Docker 底层依赖轻量虚拟机，无法直接使用 `--net=host`，必须采用上述 `ports` 显式端口映射机制。需要注意，Bridge 虚拟网桥会丢弃 `239.255.255.250:3702` 组播报文，因此在该模式下 ONVIF 摄像头无法通过广播自动搜寻，需在 Web 界面采用“手动添加”指定 IP:Port 方式接入。
  > - **Linux 裸机生产环境**：强烈推荐使用 `network_mode: host`。既能消除数百个 RTP 收流端口对 `iptables` 规则的压力，又能保证 WS-Discovery 组播直接送达主机网卡，实现局域网摄像头秒级一键搜寻。
...
| **功能闭环** | GB28181 与 ONVIF 双协议接入 | 设备上线注册成功、点播 HLS/WebRTC 成功、录像计划可落盘；支持 ONVIF 局域网探测/添加、Profile 同步、流代理播放与 PTZ 控制 | 接入国标模拟器与物理 ONVIF 摄像头执行推流、播放与云台控制验证。 |
