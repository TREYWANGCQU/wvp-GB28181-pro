...
### 0.1 已验证事实 (Fact)
1. **主开发机（Windows 11）环境**：拥有原生 JDK 21（[pom.xml](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/pom.xml%23L63)）、Maven 3.8+、Node.js（>= 18，兼容 `--openssl-legacy-provider`）及 Maven/npm 本地完整依赖缓存；项目版本为 `2.7.4`，Maven 默认产物为 `target/wvp-pro-2.7.4.jar`。
2. **配合开发机（iMac 26.1）环境**：搭载 macOS 系统与 Colima 容器守护进程（Docker CLI），原生支持 Docker Buildx 容器化跨架构构建（[doc/reaticle_docs/compile.md](Compile-and-Dev-Guide)）。
3. **架构与系统依赖**：
...
4. **数据库初始化脚本位置**：基准初始化建表脚本为 [数据库/2.7.4/初始化-mysql-2.7.4.sql](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/%E6%95%B0%E6%8D%AE%E5%BA%93/2.7.4/%E5%88%9D%E5%A7%8B%E5%8C%96-mysql-2.7.4.sql)，ONVIF 协议增量建表脚本为 [数据库/2.7.4/增量-onvif.sql](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/%E6%95%B0%E6%8D%AE%E5%BA%93/2.7.4/%E5%A2%9E%E9%87%8F-onvif.sql)（包含 `wvp_onvif_device` 与 `wvp_onvif_channel` 表）。两者在同步前必须合并，以生成完整的 `init.sql`。
...
使用项目附带的 [docker/docker-compose.aio.yml](https://github.com/TREYWANGCQU/wvp-GB28181-pro/blob/master/docker/docker-compose.aio.yml)：
```bash
